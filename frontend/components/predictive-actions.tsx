...
const getActionStyles = (type: PredictiveAction['type'], urgent?: boolean, actionCategory?: string) => {
  ...
  switch (type) {
    case 'primary':
      return `${baseClasses} bg-violet-25 hover:bg-violet-50 text-violet-800 border-violet-300 hover:border-violet-400 focus:ring-violet-500/20`
    case 'secondary':
      return `${baseClasses} bg-white hover:bg-violet-25 text-violet-700 border-violet-200 hover:border-violet-300 focus:ring-violet-500/20`
    case 'info':
      return `${baseClasses} bg-white hover:bg-violet-25 text-violet-600 border-violet-200 hover:border-violet-300 focus:ring-violet-500/20`
    default:
      return `${baseClasses} bg-white hover:bg-violet-25 text-violet-600 border-violet-200 hover:border-violet-300 focus:ring-violet-500/20`
  }
}
...
export default function PredictiveActions({ actions, onActionClick }: PredictiveActionsProps) {
  ...
  const secondaryActions = actions.filter(a => a.type === 'secondary' || a.type === 'info')
              ...
              {primaryActions.map((action) => (
                <Tooltip key={action.id}>
                  <TooltipTrigger asChild>
                    <button
                      className={getActionStyles(action.type, action.urgent, action.category)}
                      onClick={() => {
                        action.action()
                    ...
                    >
                      ...
                      {action.urgent && <AlertTriangle className="w-3 h-3 flex-shrink-0 text-violet-500" />}
                    </button>
                  </TooltipTrigger>
                  <TooltipContent 
                    side="bottom" 
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="text-xs">
                        ...
                        <ul className="space-y-0.5 text-slate-600">
                          {action.reasoning.slice(0, 3).map((reason, index) => (
...
export function generatePredictiveActions(
  ...
  // Contact Vendor (More accessible - available for any potential issues)
  if (errors > 0 || warnings > 1 || confidenceScore < 85) {
